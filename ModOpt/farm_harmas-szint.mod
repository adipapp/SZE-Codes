...
s.t. DailyStartBalance{d in 2..days}:
	dailyStartBalance[d] = dailyStartBalance[d-1] - sum{f in Farmers} boughtFarmers[d-1, f] * price[f] + sum{f in Farmers} sum{delta in 1..d-ready[f]-1} boughtFarmers[delta, f] * dailyIncome[f];
...
param ready := Peasant 1 Plower 3 Harvester 5;
